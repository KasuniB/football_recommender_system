...
# Create a dictionary of player and their ID
players = []
for idx in range(len(df)):
    players.append(df.loc[idx, 'Player'] + '({})'.format(df.loc[idx, 'Squad']))
...
def getRecommendations(df, metric, df_type, league='All', comparison='All positions', count=5):
    if df_type == 'outfield':
